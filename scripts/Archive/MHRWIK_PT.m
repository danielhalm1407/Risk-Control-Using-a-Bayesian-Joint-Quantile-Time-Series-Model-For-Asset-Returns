function [holdbeta_minus, holdbeta_plus, holdq_minus, holdq_plus, holdtheta_minus, holdtheta_plus, holdtheta_minus1, holdtheta_plus1, holdmu, holdsigvar, holdloglike, holdpredpdf, accept, aveJD, TIME] = MHRWIK_PT(dat, alpha_minus, alpha_plus, numberofits, burnin, every, type, heat)
%************ INPUTS *****************************************************
% dat
% numberofits
% burnin
...
ytot        = xtot - Median;
inSampleObs = ceil(size(ytot,1)- size(ytot,1)/2); % Number of in sample observations
y           = ytot(:, :);
% yin         = ytot(1: inSampleObs, 1);     %%% fit
% yout        = ytot(inSampleObs+1: end, 1); %%% predictive
yin         = ytot(1: inSampleObs+1, 1);     %%% fit
yout        = ytot(inSampleObs+2: end, 1); %%% predictive
...
%         %        plot(sigmasq)
    ...
    if ( (it > burnin) && (mod(it - burnin, every) == 0) ) %% checks if the number of iterations after burn in is divisible by the thinning parameter
                ...
                if ( type == 1 )
                [q_minus_out, q_plus_out, theta_minus_out, theta_plus_out]  = q_norm_BJSAV(yout, alpha_minus, alpha_plus, param);
                elseif ( type == 2 )
                [q_minus_out, q_plus_out, theta_minus_out, theta_plus_out]  = q_norm_BJSSV(yout, alpha_minus, alpha_plus, param);
                elseif ( type == 3 )
                [q_minus_out, q_plus_out, theta_minus_out, theta_plus_out]  = q_norm_BJGJR(yout, alpha_minus, alpha_plus, param);
                elseif ( type == 4 )
                [q_minus_out, q_plus_out, theta_minus_out, theta_plus_out]  = q_norm_BJSAVL(yout, alpha_minus, alpha_plus, param);
                end
                 
                
         for i=1:length(yout)

%           holdpredpdf(i) = LK_norm(q_minus_out(i, :), q_plus_out(i, :), theta_minus_out(i, :), theta_plus_out(i, :), sigmasq_out(i, 1), yout(i, 1), alpha_minus, alpha_plus);
            holdpredpdf((it - burnin) / every, i)  = LK_norm(q_minus_out(i, :), q_plus_out(i, :), theta_minus_out(i, :), theta_plus_out(i, :), sigmasq_out(i, 1), yout(i, 1), alpha_minus, alpha_plus);
            % need to add a holdpred_q_minus 
