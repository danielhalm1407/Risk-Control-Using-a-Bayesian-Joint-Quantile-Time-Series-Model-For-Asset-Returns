function [holdbeta_minus, holdbeta_plus, holdq_minus, holdq_plus, ...
        holdtheta_minus, holdtheta_plus, holdtheta_minus1, holdtheta_plus1, ...
        holdmu, holdsigvar, holdloglike, holdpredpdf, ...
        holdq_minus_forecast, holdq_plus_forecast, accept, aveJD, TIME, ...
        holdtheta_minus_forecast, holdtheta_plus_forecast] = ...
    MHRWIK_PT_forecasts(dat, alpha_minus, alpha_plus, numberofits, burnin, every, type, heat, inSampleObs)
%************ INPUTS *****************************************************
% dat
% numberofits
% burnin
...
xtot = dat;
xtot(xtot(:, 1)==0,:) = []; % removes rows with zero returns
xtot(isnan(xtot)) = []; % removes rows with NaN values

Median = prctile(xtot(:, 1), 50); % computes the median of the first column
ytot   = xtot - Median; % centers the returns

% inSampleObs is now a direct input parameter
y      = ytot(:, :);
yin    = ytot(1:inSampleObs, 1);     % in-sample (fit)
yout   = ytot(inSampleObs+1:end, 1); % out-of-sample (predictive)
...
holdq_minus      = zeros(numberofits, length(yin), length(alpha_minus)-2);
holdq_plus       = zeros(numberofits, length(yin), length(alpha_plus)-2);

% create variables to hold forecast quantiles
num_forecast_samples = floor(numberofits/every);
holdq_minus_forecast = zeros(num_forecast_samples, length(yout), length(alpha_minus)-2);
holdq_plus_forecast  = zeros(num_forecast_samples, length(yout), length(alpha_plus)-2);
% create variables to hold forecasted theta values
holdtheta_minus_forecast = zeros(num_forecast_samples, length(yout), fin1_minus);
holdtheta_plus_forecast  = zeros(num_forecast_samples, length(yout), fin1_plus);


holdtheta_minus  = zeros(numberofits, length(yin), fin1_minus);
holdtheta_plus   = zeros(numberofits, length(yin), fin1_plus);
...
holdpredpdf      = zeros(numberofits, length(yout));
...
sigmasq      = ones(length(yin), numbofchains);

q_minus = zeros(length(yin), length(alpha_minus) - 2, numbofchains);
q_plus  = zeros(length(yin), length(alpha_plus) - 2, numbofchains);
theta_minus = zeros(length(yin), length(alpha_minus) - 1, numbofchains);
theta_plus  = zeros(length(yin), length(alpha_plus) - 1, numbofchains);
loglike     = zeros(1, numbofchains);

for chain = 1:numbofchains
    param = [beta_minus(:, chain)' beta_plus(:, chain)' theta_minus1(:, chain)' theta_plus1(:, chain)'];
    if ( type == 1 )
        [q_minus(:, :, chain), q_plus(:, :, chain), theta_minus(:, :, chain), theta_plus(:, :, chain)] = q_norm_BJSAV(yin, alpha_minus, alpha_plus, param);
    elseif ( type == 2 )
        [q_minus(:, :, chain), q_plus(:, :, chain), theta_minus(:, :, chain), theta_plus(:, :, chain)] = q_norm_BJSSV(yin, alpha_minus, alpha_plus, param);
    elseif  ( type == 3 )
        [q_minus(:, :, chain), q_plus(:, :, chain), theta_minus(:, :, chain), theta_plus(:, :, chain)] = q_norm_BJGJR(yin, alpha_minus, alpha_plus, param);
     elseif  ( type == 4 )
        [q_minus(:, :, chain), q_plus(:, :, chain), theta_minus(:, :, chain), theta_plus(:, :, chain)] = q_norm_BJSAVL(yin, alpha_minus, alpha_plus, param);
    end
    loglike(chain) = LK_norm(q_minus(:, :, chain), q_plus(:, :, chain), theta_minus(:, :, chain), theta_plus(:, :, chain), sigmasq(:, chain), yin, alpha_minus, alpha_plus);
end
...
%         %        plot(sigmasq)
    ...
    for chain = 1:numbofchains
        ...
        for i=1:size(theta_minus1, 1)
            ...
            if ( type == 1 )
                [newq_minus, newq_plus, newtheta_minus, newtheta_plus] = q_norm_BJSAV(yin, alpha_minus, alpha_plus, param);
            elseif  ( type == 2 )
                [newq_minus, newq_plus, newtheta_minus, newtheta_plus] = q_norm_BJSSV(yin, alpha_minus, alpha_plus, param);
            elseif ( type == 3 )
                [newq_minus, newq_plus, newtheta_minus, newtheta_plus] = q_norm_BJGJR(yin, alpha_minus, alpha_plus, param);
             elseif  ( type == 4 )
                [newq_minus, newq_plus, newtheta_minus, newtheta_plus] = q_norm_BJSAVL(yin, alpha_minus, alpha_plus, param);
            end
            newloglike = LK_norm(newq_minus, newq_plus, newtheta_minus, newtheta_plus, sigmasq(:, chain), yin, alpha_minus, alpha_plus);
    ...
    if ( (it > burnin) && (mod(it - burnin, every) == 0) ) %% checks if the number of iterations after burn in is divisible by the thinning parameter
        

        sigmasq_out = sigmasq(2:end, top_chain);

        % Update param so that theta_minus1 is set to the last in-sample theta_minus value
        param_forecast = param;
        % theta_minus1 is after beta_minus and beta_plus in param vector
        % param = [beta_minus, beta_plus, theta_minus1, theta_plus1]
        % theta_minus1 indices: (length(beta_minus) + length(beta_plus) + 1):(length(beta_minus) + length(beta_plus) + fin1_minus)
        idx_theta_minus1 = length(beta_minus(:, top_chain)) + length(beta_plus(:, top_chain));
        % Set to last in-sample theta_minus for each tail (for BJSAV, usually fin1_minus=1)
        for k = 1:fin1_minus
            param_forecast(idx_theta_minus1 + k) = theta_minus(end, k, top_chain);
        end
        % print the theta_minus values being input for forecasting
        % disp(['theta_minus1 for forecasting: ' num2str(param_forecast(idx_theta_minus1 + 1:idx_theta_minus1 + fin1_minus))]);
        % hmmm, using the correct ones but for some reason the forecast thetas are coming out very strange

        if ( type == 1 )
            [q_minus_out, q_plus_out, theta_minus_out, theta_plus_out]  = q_norm_BJSAV(yout, alpha_minus, alpha_plus, param_forecast);
        elseif ( type == 2 )
            [q_minus_out, q_plus_out, theta_minus_out, theta_plus_out]  = q_norm_BJSSV(yout, alpha_minus, alpha_plus, param_forecast);
        elseif ( type == 3 )
            [q_minus_out, q_plus_out, theta_minus_out, theta_plus_out]  = q_norm_BJGJR(yout, alpha_minus, alpha_plus, param_forecast);
        elseif ( type == 4 )
            [q_minus_out, q_plus_out, theta_minus_out, theta_plus_out]  = q_norm_BJSAVL(yout, alpha_minus, alpha_plus, param_forecast);
        end
        % These will be the last forecasted values from the final iteration

        idx = (it - burnin) / every;
        holdq_minus_forecast(idx, :, :) = q_minus_out;
        holdq_plus_forecast(idx, :, :)  = q_plus_out;
        holdtheta_minus_forecast(idx, :, :) = theta_minus_out;
        holdtheta_plus_forecast(idx, :, :)  = theta_plus_out;
    % PDF calculation for out-of-sample is not needed, so it is commented out
    % for i=1:length(yout)
    %     holdpredpdf(idx, i)  = LK_norm(q_minus_out(i, :), q_plus_out(i, :), theta_minus_out(i, :), theta_plus_out(i, :), sigmasq_out(i, 1), yout(i, 1), alpha_minus, alpha_plus);
    % end
